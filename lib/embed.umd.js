/**
 * @formcrafts/embed
 * Copyright (c) 2024-2026, Nishant Agrawal. (MIT Licensed)
...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.formcrafts = {}));
    ...
    ***************************************************************************** */
    ...
    function getPageUrl(override) {
        var _a;
        var explicitUrl = override ? asPageUrl(override) : null;
        if (explicitUrl)
            return explicitUrl;
        var current = window;
        var referrer = null;
        while (true) {
            try {
                var url = asPageUrl(current.location.href);
                if (debug)
                    console.log("Current URL:", current.location.href);
                if (debug)
                    console.log("Referrer:", current.document.referrer);
                if (url)
                    return url;
                // Retain a fallback if the next parent is inaccessible.
                referrer = (_a = asPageUrl(current.document.referrer)) !== null && _a !== void 0 ? _a : referrer;
    ...
    if (typeof window !== "undefined") {
        ...
        window.on = window.on || function (event, callback) {
    ...
    function dispatchGlobalEvent(eventName, form_id, data) {
        if (typeof window !== "undefined") {
            var globalEventName = "formcrafts:".concat(eventName);
            // Dispatch DOM event for window.addEventListener
            var customEvent = new CustomEvent(globalEventName, {
                detail: { form_id: form_id, data: data },
            });
            window.dispatchEvent(customEvent);
            // Also support legacy window.on method
            if (window._formcraftsGlobalEvents) {
                var callbacks = window._formcraftsGlobalEvents[globalEventName];
                if (callbacks && callbacks.length > 0) {
                    callbacks.forEach(function (callback) {
                        try {
                            callback({ form_id: form_id, data: data });
                        }
                        catch (error) {
     ...
     */
    function sameFormUrl(a, b) {
        if (!a || !b)
            return false;
        try {
            var urlA = new URL(a, window.location.href);
            var urlB = new URL(b, window.location.href);
        ...
        window.addEventListener("message", function (event) {
            ...
            if (event.data.type === "load") {
                ...
                if (iframe.contentWindow) {
                    ...
                    var pageUrl = getPageUrl(options.pageUrl);
                    if (pageUrl) {
                        iframe.contentWindow.postMessage({
                            type: "url",
                            content: pageUrl,
            ...
            if (event.data.type === "submit:success" ||
                ...
                if (events_2["submit:success"]) {
                    events_2["submit:success"].forEach(function (callback) {
                        return callback(event.data.content);
                    });
                }
            ...
            if (event.data.type === "close" && type === "popup" && options) {
                ...
                var existingModal = iframe.closest(".fc-modal");
    ...
    function createInlineForm(options) {
        ...
        var iframeSrc = buildIframeSrc(options);
    ...
    function createPopupReturn(drawer, modal, config) {
    ...
    function createPopup(options) {
        ...
        var modalId = "fc-modal-".concat(options.form, "-").concat(options.idSuffix ? options.idSuffix : Math.floor(Math.random() * 10000));
        // Check if modal exists
        if (document.getElementById(modalId)) {
            var existingModal = document.getElementById(modalId);
            var existingDrawer = existingModal.drawerInstance;
            return createPopupReturn(existingDrawer, existingModal);
        }
        // Get / create style
        ...
        return createPopupReturn(myDrawer, modal);
    }
